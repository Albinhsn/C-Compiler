...
#define sta_arena_push_array(arena, type, count)                               \
  (type *)sta_arena_push((arena), sizeof(type) * (count))
#define sta_arena_push_struct(arena, type)                                     \
  sta_arena_push_array((arena), type, 1)
