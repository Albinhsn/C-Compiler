...
void sta_arena_pop(Arena *arena, u64 size) { arena->ptr -= size; }
void sta_arena_init_heap(Arena *arena, u64 size) {
  arena->ptr = 0;
  arena->maxSize = size;
...
bool sta_destroyLogger(Logger *logger) { return fclose(logger->filePtr); }
...
void sta_strsplit(Arena *arena, StringArray *res, String *s1, char c) {
  ...
  for (u64 idx = 0; idx < s1->len; idx++) {
    if (s1->buffer[idx] == c) {
      ...
      res->str[res->len] =
          (String){.len = idx - prevIdx, .buffer = &s1->buffer[prevIdx]};
      res->len++;
      res->capacity++;
  ...
  String tmp =
      (String){.len = s1->len - prevIdx, .buffer = &s1->buffer[prevIdx]};
