...
void init_scanner(Scanner *scanner, Arena *arena, String *literal,
                  const char *filename) {
...
static bool match_substring(String *literal, int index, const char *to_match,
                            int length) {
...
static TokenType get_keyword(String literal) {
  static const char *keywords[] = {
      ...
      "unsinged",  "void",     "volatile", "while",  "_Bool",   "_Complex",
      "_Imaginary"};
  static TokenType tokens[] = {
      ...
      TOKEN_VOLATILE,  TOKEN_WHILE,    TOKEN_BOOL,     TOKEN_COMPLEX,
      TOKEN_IMAGINARY,
  };
  u32 number_of_keywords = ArrayCount(keywords);
  for (u32 i = 0; i < number_of_keywords; i++) {
    if (literal.len == strlen(keywords[i]) &&
        strncmp(keywords[i], literal.buffer, literal.len) == 0) {
      return tokens[i];
    }
  }
...
static Token *parse_keyword(Scanner *scanner) {
  ...
  u8 current = advance(scanner);
  while (!is_out_of_bounds(scanner) &&
         (isdigit(current) || isalpha(current) || current == '_')) {
    current = advance(scanner);
  }
  ...
  return create_token(scanner->arena, get_keyword(literal), literal,
                      scanner->line, scanner->index);
}
...
static Token *parse_hex(Scanner *scanner, i32 index) {
  ...
  while (!is_out_of_bounds(scanner) &&
         (isdigit(current) || is_valid_hex(current))) {
    current = advance(scanner);
  }
  scanner->index--;
  String literal = {};
...
Token *parse_token(Scanner *scanner) {
  ...
  case '{': {
    ...
    literal.buffer = "{";
    literal.len = 1;
    return create_token(scanner->arena, TOKEN_LEFT_BRACE, literal,
                        scanner->line, scanner->index);
  }
  case '}': {
    String literal = {};
    literal.buffer = "}";
    literal.len = 1;
    return create_token(scanner->arena, TOKEN_RIGHT_BRACE, literal,
                        scanner->line, scanner->index);
  }
  case '[': {
    String literal = {};
    literal.buffer = "[";
    literal.len = 1;
    return create_token(scanner->arena, TOKEN_LEFT_BRACKET, literal,
                        scanner->line, scanner->index);
  }
  case '\'': {
  ...
  case ']': {
    ...
    literal.buffer = "]";
    literal.len = 1;
    return create_token(scanner->arena, TOKEN_RIGHT_BRACKET, literal,
                        scanner->line, scanner->index);
  }
  case ';': {
    String literal = {};
  ...
  case '(': {
    ...
    literal.buffer = "(";
    literal.len = 1;
    return create_token(scanner->arena, TOKEN_LEFT_PAREN, literal,
                        scanner->line, scanner->index);
  }
  case ')': {
    String literal = {};
    literal.buffer = ")";
    literal.len = 1;
    return create_token(scanner->arena, TOKEN_RIGHT_PAREN, literal,
                        scanner->line, scanner->index);
  }
  case '/': {
    String literal = {};
    if (match_next(scanner, '=')) {
      literal.buffer = "/=";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_AUGMENTED_SLASH, literal,
                          scanner->line, scanner->index);
    }
    literal.buffer = "/";
    literal.len = 1;
  ...
  case '*': {
    ...
    if (match_next(scanner, '=')) {
      literal.buffer = "*=";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_AUGMENTED_STAR, literal,
                          scanner->line, scanner->index);
    }
    literal.buffer = "*";
    literal.len = 1;
  ...
  case '&': {
    ...
    if (match_next(scanner, '=')) {
      literal.buffer = "&=";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_AUGMENTED_AND, literal,
                          scanner->line, scanner->index);
    } else if (match_next(scanner, '&')) {
      literal.buffer = "&&";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_AND_LOGICAL, literal,
                          scanner->line, scanner->index);
    }
  ...
  case '|': {
    ...
    if (match_next(scanner, '=')) {
      literal.buffer = "|=";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_AUGMENTED_OR, literal,
                          scanner->line, scanner->index);
    } else if (match_next(scanner, '|')) {
      literal.buffer = "||";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_OR_LOGICAL, literal,
                          scanner->line, scanner->index);
    }
  ...
  case '!': {
    ...
    if (match_next(scanner, '=')) {
      literal.buffer = "!=";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_BANG_EQUAL, literal,
                          scanner->line, scanner->index);
    }
    literal.buffer = "!";
    literal.len = 1;
  ...
  case '>': {
    ...
    if (match_next(scanner, '=')) {
      literal.buffer = ">=";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_GREATER_EQUAL, literal,
                          scanner->line, scanner->index);
    } else if (match_next(scanner, '>')) {
      literal.buffer = ">>";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_SHIFT_RIGHT, literal,
                          scanner->line, scanner->index);
    }
    literal.buffer = ">";
    literal.len = 1;
  ...
  case '-': {
    ...
    if (match_next(scanner, '-')) {
      literal.buffer = "--";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_DECREMENT, literal,
                          scanner->line, scanner->index);
    } else if (match_next(scanner, '=')) {
      literal.buffer = "-=";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_AUGMENTED_MINUS, literal,
                          scanner->line, scanner->index);
    }
    literal.buffer = "-";
    literal.len = 1;
  ...
  case '+': {
    ...
    if (match_next(scanner, '+')) {
      literal.buffer = "++";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_INCREMENT, literal,
                          scanner->line, scanner->index);
    } else if (match_next(scanner, '=')) {
      literal.buffer = "-=";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_AUGMENTED_PLUS, literal,
                          scanner->line, scanner->index);
    }
    literal.buffer = "+";
    literal.len = 1;
  ...
  case '=': {
    ...
    if (match_next(scanner, '=')) {
      literal.buffer = "==";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_EQUAL_EQUAL, literal,
                          scanner->line, scanner->index);
    }
    literal.buffer = "=";
    literal.len = 1;
  ...
  case '<': {
    ...
    if (match_next(scanner, '=')) {
      literal.buffer = "<=";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_LESS_EQUAL, literal,
                          scanner->line, scanner->index);
    } else if (match_next(scanner, '<')) {
      literal.buffer = "<<";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_SHIFT_LEFT, literal,
                          scanner->line, scanner->index);
    }
    literal.buffer = "<";
    literal.len = 1;
  ...
  case '^': {
    ...
    if (match_next(scanner, '=')) {
      literal.buffer = "^=";
      literal.len = 2;
      return create_token(scanner->arena, TOKEN_AUGMENTED_XOR, literal,
                          scanner->line, scanner->index);
    }
    literal.buffer = "^";
    literal.len = 1;
  ...
  case '.': {
    ...
    if (isdigit(current_char(scanner))) {
      ...
      literal.buffer = "...";
      literal.len = 3;
      return create_token(scanner->arena, TOKEN_ELLIPSIS, literal,
                          scanner->line, scanner->index);
    }
    literal.buffer = ".";
    literal.len = 1;
  ...
  default: {
    printf("%c %d\n", scanner->input->buffer[scanner->index],
           scanner->input->buffer[scanner->index]);
